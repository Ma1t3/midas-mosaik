...
class ConstraintVoltageTimed(Constraint):
    """
    TAB4
    The voltage value of a generator is allowed to move between 85% and 90% or
    110% and 115% of its expected value over the duration of 1 minute
    """
    ...
    def __init__(self, constraint_container):
        ...
        self.allowed_duration = datetime.timedelta(minutes=1)
        self.triggered = None
    ...
    def check(self, time):
        ...
        if np.isnan(voltage):
            ...
            self.satisfied = True
        elif not 0.9 < voltage < 1.1:
            # Violation
            if not self.triggered:
                # First violation
                self.triggered = time
            else:
                # Persisting violation
                if (
                    time - self.triggered
                ) > self.allowed_duration.total_seconds():
                    # Violation consisting for > 1 minute
